...
const AdComponent = ({ id }) => (
  <div key={`ad-${id}`} className="my-8 p-6 bg-gray-100 dark:bg-gray-800 rounded-lg text-center">
...
export async function generateMetadata({ params }) {
  ...
  const res = await fetch(`${BaseApi}/blogs/${blogName}`, { cache: "no-store" });
...
export default async function BlogPage({ params }) {
  ...
  let adCounter = 0;
  let currentCount = 0;
  let nextAdAt = getRandomInterval(AD_CONFIG.MIN_INTERVAL, AD_CONFIG.MAX_INTERVAL);
  ...
  paragraphs.forEach((paragraph, index) => {
    ...
    if (currentCount === nextAdAt) {
      ...
      currentCount = 0;
      nextAdAt = getRandomInterval(AD_CONFIG.MIN_INTERVAL, AD_CONFIG.MAX_INTERVAL);
    }
  });

  return (
    <div className="px-6 md:px-64 py-8">
      <Head>
